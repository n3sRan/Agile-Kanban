...
export class CommentController {
    ...
    async createComment(@Param('taskId') taskId: string, @Body() commentData: any) {
        const comment = await this.commentService.create(taskId, commentData);
    ...
    async getComments(@Param('taskId') taskId: string) {
        const comments = await this.commentService.list(taskId);
        return comments;
    }
    ...
    async likeComment(@Param('commentId') commentId: string) {
        const likedComment = await this.commentService.like(commentId);
